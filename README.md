...
> not for long. **Nothing since has been on hardware at all.** 3.1.2 builds end
...
| File | What it is |
| --- | --- |
| `FunKey-sdcard-Starling-3.1.2-RG_Nano.img` | Full SD card image — installs Starling, erasing the card |
| `FunKey-rootfs-Starling-3.1.2-RG_Nano.fwu` | Firmware update — replaces the OS in place, keeping games and saves |
| `FunKey-sdk-Starling-3.1.2.tar.gz` | Cross-toolchain, only needed to build software *for* the console |
...
```bash
sudo dd if=FunKey-sdcard-Starling-3.1.2-RG_Nano.img of=/dev/sdX bs=4M conv=fsync status=progress
```
...
3. Copy `FunKey-rootfs-Starling-3.1.2-RG_Nano.fwu` onto the drive that appears.
